extends Module
class_name DefaultUI_DialogModule
...
func append_dialog(__dialog: Dictionary[StringName, PackedScene], __log: bool = false) -> void:
	...
	for __alias: StringName in __dialog.keys():
		if __alias.is_empty():
			printerr("DefaultUI_DialogModule: dialog with no alias. Skipping it.")
			continue
		if __dialog[__alias] == null:
			printerr("DefaultUI_DialogModule: dialog '%s' has no scene. Skipping it." % __alias)
			continue
		if __alias in __loaded_dialogs:
			printerr("DefaultUI_DialogModule: dialog '%s' has the same alias as a previously loaded action. Please unload that one then append it." % __alias)
			continue
	...
	if __log:
		...
		print("DefaultUI_DialogModule: %s loaded:\n- %s" % [__loaded_dialogs.size(), \
...
func open(__alias: StringName, __param: Dictionary) -> bool:
	...
	if __scn == null:
		printerr("DefaultUI_DialogModule: dialog '%s' not found or loaded." % __alias)
		return false
