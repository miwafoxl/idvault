...
@export var loaded_panels: Dictionary[StringName, DefaultUI_Panel]

#region OVERRIDES

func update() -> void:
	for __panel: DefaultUI_Panel in loaded_panels.values():
		__panel.update()

func update_selection() -> void:
	for __panel: DefaultUI_Panel in loaded_panels.values():
		__panel.update_selection()
...
func update_panels() -> void:
	for __control: Control in %HBOX_C.get_children():
		...
		if __control is DefaultUI_ItemListPanel:
			...
			loaded_panels.set(&"ItemListPanel", __panel)
		if __control is DefaultUI_ItemOverview:
			var __panel: DefaultUI_ItemOverview = __control
			loaded_panels.set(&"ItemOverview", __panel)
		
func update_signals() -> void:
	for __control: Control in %HBOX_C.get_children():
		if __control is DefaultUI_Panel:
			__control.trigger.connect(trigger.emit)
