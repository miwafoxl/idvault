...
func build(__param: Dictionary) -> ContextMenu:
	...
	if __items_size > 1:
		...
		__menu.set_item_metadata(-1, {&"items.dialog.selected_common_links": {} })
	else:
		...
		__menu.set_item_metadata(-1, {&"items.dialog.editor_links": {} })
