...
func run(__mod_item: ItemModule, __mod_action: ActionModule) -> int:
	#region PROCEDURES
	procedures = [
		__mod_action.run.bindv([&"items.append.items", {"count": 50}]),
		func() -> bool: return __mod_item.unordered_items.size() == 50,
		__mod_action.run.bindv([&"items.stage.unordered", {}]),
		__mod_action.run.bindv([&"items.select.by_item_index", {"item_idx": range(50)}]),
		func() -> bool: return __mod_item.selected_items.size() == 50,
		__mod_action.run.bindv([&"property.append.selected", {"properties" : [
			Display.new("test")]} ]),
		func() -> bool: return __mod_item.item_cache["by_property_id"].size() == 50,
