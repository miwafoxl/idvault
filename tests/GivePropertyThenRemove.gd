...
## Adds an item, gives it a property and remove that property afterwards
func run(__mod_item: ItemModule, __mod_action: ActionModule) -> int:
	...
	procedures = [
		...
		__mod_action.run.bindv([&"property.append.selected", {"properties" : [
			Display.new("test")]} ]),
		func() -> bool: return __mod_item.selected_items[0].properties.size() > 0,
		__mod_action.run.bindv([&"property.remove.selected_index", {"prop_idx": [0]}]),
		func() -> bool: return __mod_item.selected_items[0].properties.size() == 0,
